...
export async function waitForVideoGeneration(
	...
	for (let attempt = 0; attempt < maxAttempts; attempt++) {
		...
		Logger.info(`Bookoly: waitForVideoGeneration--> ${JSON.stringify(response)}`, {response});
	...
	const error = new NodeOperationError(ctx.getNode(), 'Timeout while waiting for video generation to finish');
