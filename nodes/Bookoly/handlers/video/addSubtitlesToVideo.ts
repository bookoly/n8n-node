...
export async function addSubtitlesToVideo(
	...
	Logger.info(`Adding subtitles to video initiated`, {
		videoName: name,
		videoUrl: url,
	});

	const response = await apiRequest(ctx, 'POST', 'add-subtitles-to-video', body);
	Logger.info(`Subtitles added to video successfully`, { response });
