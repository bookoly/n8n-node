...
export async function combineSounds(
	ctx: IExecuteFunctions,
	itemIndex: number,
): Promise<any> {
	const name = ctx.getNodeParameter('name', itemIndex) as string;
	const segments = ((ctx.getNodeParameter('segmentList', itemIndex) as any).segmentValues || []) as Array<{ src: string }>;
	...
	if (wait && response?.id) {  // Check for response.id directly, not response.sound.id
		...
		return await waitForSound(ctx, response.id);  // Use waitForSound helper with response.id
	}
